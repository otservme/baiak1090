...
local cfgItems = {{7423, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(7423) and player:getStorageValue(15140) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15140) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou um King Club.")
