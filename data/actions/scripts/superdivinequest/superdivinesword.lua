...
local cfgItems = {{23550, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(23550) and player:getStorageValue(15144) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15144) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou uma Super Divine Sword.")
