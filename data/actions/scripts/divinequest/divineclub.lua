...
local cfgItems = {{23544, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(23544) and player:getStorageValue(15141) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15141) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou um Divine Club.")
