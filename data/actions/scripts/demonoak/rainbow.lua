local cfgItems = {{8905, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(8905) and player:getStorageValue(15127) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15127) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou uma Rainbow Shield.")
