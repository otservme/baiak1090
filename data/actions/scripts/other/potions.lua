...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif itemId == manaPot then
		if not doTargetCombatMana(0, target, 175, 325, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatSpiritPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 550, 750, CONST_ME_MAGIC_BLUE) or not doTargetCombatMana(0, target, 250, 400, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatManaPot then
		...
		if not doTargetCombatMana(0, target, 650, 950, CONST_ME_MAGIC_BLUE) then
			return false
		end
		player:addCondition(exhaust)
		target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
	elseif itemId == ultimateHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 950, 1250, CONST_ME_MAGIC_BLUE) then
			return false
		end
