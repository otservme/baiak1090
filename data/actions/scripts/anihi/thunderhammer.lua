...
local cfgItems = {{2421, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(2421) and player:getStorageValue(15129) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15129) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou um Thunder Hammer.")
