...
local cfgItems = {{9777, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(9777) and player:getStorageValue(15128) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15128) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou uma Yalahari Leg Piece.")
