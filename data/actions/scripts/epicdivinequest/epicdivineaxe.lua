...
local cfgItems = {{15404, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(15404) and player:getStorageValue(15145) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15145) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou um Epic Divine Axe.")
