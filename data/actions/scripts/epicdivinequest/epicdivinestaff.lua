...
local cfgItems = {{15400, 1}}

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if item:getUniqueId(15400) and player:getStorageValue(15145) == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce ja escolheu a sua recompensa.")
    elseif player:getStorageValue(15145) <= 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "Voce achou uma Epic King Staff.")
